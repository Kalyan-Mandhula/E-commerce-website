...
const AdminCreateProductComponent = ({ CreateProduct  }) => {
...
const uploadImagesCloudinaryApiRequest = (images) => {
 ...
 const AddNewAttr = async(category,key,value)=>{
  const {data} =await axios.post("/api/categories/attributes",{category : category.name , key ,value})
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === true) { 
        const FinalProduct = {
         ...
         CreateProduct(FinalProduct).then((res)=>console.log(res)).catch((err)=>console.log(err))
...
export default AdminCreateProductComponent;
